...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, topic_representation,
  ...
  Args:
    ...
    topic_representation: topic representation of para_num paragraph in an article [batch_size x para_num x topic_dim]
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    topic_attn_size = topic_representation.get_shape()[2].value

    # Reshape encoder_states (need to insert a dim)
    encoder_states = tf.expand_dims(encoder_states, axis=2) # now is shape (batch_size, attn_len, 1, attn_size)
    topic_representation = tf.expand_dims(topic_representation, axis=2)
    ...
    # Get the topic encoder state feature
    W_h_topic = variable_scope.get_variable("W_h_topic", [1, 1, topic_attn_size, topic_attn_size])
    topic_encoder_features = nn_ops.conv2d(topic_representation, W_h_topic, [1, 1, 1, 1], "SAME")  # shape (batch_size,attn_length,1,attention_vec_size)

    # Get the weight vectors v and w_c (w_c is for coverage)
    v = variable_scope.get_variable("v", [attention_vec_size])
    v_t = variable_scope.get_variable("v_t", [topic_attn_size])
    if use_coverage:
    ...
    def attention(decoder_state, coverage=None):
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        if use_coverage and coverage is not None: # non-first step of coverage
          ...
          # attn_dist = tf.multiply(attn_dist, tf.squeeze(input_prob))
    ...
    def topic_attention(decoder_state):
      """Calculate the topic context vector from the decoder state.
      Returns:
        topic_context_vector: weighted sum of topic_states
      """
      with variable_scope.variable_scope("TopicAttention"):
        # Pass the decoder state through a linear layer (this is W_s s_t + b_attn in the paper)
        decoder_features = linear(decoder_state, topic_attn_size, True)  # shape (batch_size, attention_vec_size)
        decoder_features = tf.expand_dims(tf.expand_dims(decoder_features, 1), 1)  # reshape to (batch_size, 1, 1, attention_vec_size)
        ...
        # Calculate v^T tanh(W_h h_i + W_s s_t + b_attn)
        e = math_ops.reduce_sum(v_t * math_ops.tanh(topic_encoder_features + decoder_features), [2, 3])  # calculate e

        # Calculate attention distribution
        attn_dist_t = masked_attention(e)

        # Calculate the context vector from attn_dist and encoder_states
        context_vector_t = math_ops.reduce_sum(array_ops.reshape(attn_dist_t, [batch_size, -1, 1, 1]) * topic_representation, [1, 2])
        context_vector_t = array_ops.reshape(context_vector_t, [-1, topic_attn_size])

      return context_vector_t
    ...
    topic_context_vector = array_ops.zeros([batch_size, topic_attn_size])
    topic_context_vector.set_shape([None, topic_attn_size])
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      x = linear([inp] + [context_vector] + [topic_context_vector], input_size, True)
      ...
      if i == 0 and initial_state_attention:  # always true in decode mode
        with variable_scope.variable_scope(variable_scope.get_variable_scope(), reuse=tf.AUTO_REUSE): # you need this because you've already run the initial attention(...) call
          ...
          topic_context_vector = topic_attention(state)
      else:
      ...
      if pointer_gen:
        with tf.variable_scope('calculate_pgen'):
          p_gen = linear([context_vector, topic_context_vector, state.c, state.h, x], 1, True) # a scalar
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        output = linear([cell_output] + [context_vector] + [topic_context_vector], cell.output_size, True)

...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, index, vocab, topic_vocab, hps):
    ...
    self.index = index
    self.term_matrix = self.cal_term_frequency(article_words, topic_vocab)
  ...
  def cal_term_frequency(self, article_words, topic_vocab):
    term_matrix = np.zeros([self.hps.para_num, self.hps.topic_vocab_size])
    para_size = int(len(article_words) / self.hps.para_num)
    for i in range(self.hps.para_num):
      word_counts = dict(Counter(article_words[i*para_size:(i+1)*para_size]))
      for word, count in word_counts.items():
        if word in topic_vocab._word_to_id:
          index = topic_vocab.word2id(word)
          term_matrix[i][index] = count
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    self.doc_index = np.zeros((hps.batch_size), dtype=np.int32)
    self.term_frequency = np.zeros([hps.batch_size, hps.para_num, hps.topic_vocab_size])

    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      self.enc_batch[i, :] = ex.enc_input[:]
      self.enc_lens[i] = ex.enc_len
      self.doc_index[i] = ex.index
      self.term_frequency[i] = ex.term_matrix
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, topic_vocab, hps, single_pass):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract, index) = next(input_gen) # read the next example from file. article and abstract are both strings.
      ...
      example = Example(article, abstract_sentences, index, self._vocab, self._topic_vocab, self._hps) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        ...
        index = e.features.feature['index'].bytes_list.value[0].decode()
      except ValueError:
      ...
      if len(article_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        ...
        yield (article_text, abstract_text, index)

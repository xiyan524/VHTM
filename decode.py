...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab, hps):
    ...
    ckpt_path = util.load_ckpt(self._saver, self._sess, load_best=False)
  ...
  def decode(self, word_embedding):
    ...
    #load pre_train word embedding
    self._model.run_pre_train(self._sess, word_embedding)

    # load pre-trained document embedding
    doc_presentation = np.load("/home/cide/Workspace_FXY/data/doc_representation_test(bert).npy")
    ...
    while True:
      ...
      # calculation document representation
      doc_pre = self.cal_doc_pre(batch, doc_presentation)

      # Run beam search to get best Hypothesis
      best_hyp = beam_search.run_beam_search(self._sess, self._model, self._vocab, batch, doc_pre)
...
def rouge_eval(ref_dir, dec_dir):
  ...
  r = pyrouge.Rouge155('/home/cide/Workspace_FXY/ROUGE/RELEASE-1.5.5')
